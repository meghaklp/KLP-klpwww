...
begin
        for schs in SELECT s.id as id, s.name as name, s.bid as bid, c.sex as sex, count(stu.id) AS count
                 ...
                 WHERE cl.sid = s.id AND sc.clid = cl.id AND sc.stuid = stu.id AND stu.cid = c.id AND sc.ayid = $1
...
declare
        stueval RECORD;
begin
        for stueval in SELECT s.id as id, ass.id as assid, sc.clid as clid, c.sex as sex, c.mt as mt, se.grade as grade, cast(count(stu.id) as float) as cnt
...
begin
        ...
        for stueval in SELECT s.id as id, ass.id as assid, sc.clid as clid, c.sex as sex, c.mt as mt, 
          count(case when se.mark < 20 then stu.id else null end) as Rung1, 
          count(case when se.mark between 20 and 40 then stu.id else null end) as Rung2, 
          count(case when se.mark between 40 and 60 then stu.id else null end) as Rung3, 
          count(case when se.mark between 60 and 80 then stu.id else null end) as Rung4, 
          count(case when se.mark > 80 then stu.id else null end) as Rung5
FROM tb_student stu, tb_class cl, tb_student_class sc, tb_child c, tb_school s, tb_student_eval se, tb_assessment ass
WHERE cl.sid = s.id AND sc.clid = cl.id AND sc.stuid = stu.id AND stu.cid = c.id AND stu.id = se.stuid AND se.assid = ass.id AND ass.id = $1 AND sc.ayid = $2
...
-- 2009 Target reading
select agg_school_grade(24, 119);
-- 2007 NNG
...
-- 2009 Ramanagra NNG1
select agg_school_nng(19, 119);
select agg_school_nng(20, 119);
-- 2009 Ramanagra NNG2
